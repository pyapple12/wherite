...
### [O02b].[Markdown 编辑]：Markdown 语法支持和实时预览
...
数据库初始化时会自动创建 articles 表，并为 created_at 和 updated_at 字段设置默认值（当前本地时间）。同时创建两个触发器：

1. `set_created_at` - 在插入文章时自动设置创建时间和更新时间
2. `set_updated_at` - 在更新文章时自动更新更新时间
